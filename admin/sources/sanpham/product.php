...
function save_item(){
	...
	$data['color'] = magic_quote($_POST['color']);
	$data['tongratting'] = magic_quote($_POST['tongratting']);
	$data['soluotratting'] = magic_quote($_POST['soluotratting']);
	...
	if($id){		//Sửa	
		if($photo = upload_image("file", 'jpg|png|gif|JPG|PNG|GIF|jpeg|JPEG', _upload_sanpham,$file_name)){
			$data['photo'] = $photo;	
			$data['thumb'] = create_thumb($data['photo'], 341, 341, _upload_sanpham,$file_name,2);
			$data['thumbchitiet'] = create_thumb($data['photo'], 375, 205.5, _upload_sanpham,$file_name,2);
			$d->setTable('product');
			$d->setWhere('id', $id);
			$d->select();
			if($d->num_rows()>0){
...
function save_cat(){
	...
	if($id){
		if($photo = upload_image("file", 'jpg|jpeg|png|gif|JPG|JPEG|PNG|GIF', _upload_sanpham,$file_name)){
			...
			$data['photo'] = $photo;			
			$data['thumb'] = create_thumb($data['photo'], 213, 100, _upload_sanpham,$filename,2);
...
function save_photo(){
	...
	if($id){		
		...
		if($photo = upload_image("file", 'jpg|png|gif|JPG|PNG|GIF|jpeg|JPEG', _upload_sanpham,$filename)){
			$data['photo'] = $photo;
			$data['thumb'] = create_thumb($data['photo'], 1366, 670, _upload_sanpham,$filename,2);
		...
		if($download = upload_image("download", 'jpg|png|gif|JPG|PNG|GIF|jpeg|JPEG|pdf|PDF', _upload_sanpham,$filename1)){
